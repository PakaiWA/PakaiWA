...
import (
	model2 "github.com/PakaiWA/PakaiWA/internal/app/pakaiwa/delivery/model"
...
func NewAuth(usecase *usecase.UserUseCase, rateLimiterUtil *utils.RateLimiterUtil) fiber.Handler {
	return func(ctx *fiber.Ctx) error {
		request := &model2.VerifyUserRequest{Token: ctx.Get("Authorization", "NOT_FOUND")}
...
func GetUser(ctx *fiber.Ctx) *model2.Model {
	return ctx.Locals("auth").(*model2.Model)
}
