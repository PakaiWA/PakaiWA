...
func (_q *UserQuery) loadPermissions(ctx context.Context, query *PermissionQuery, nodes []*User, init func(*User), assign func(*User, *Permission)) error {
	...
	for _, n := range neighbors {
		...
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_permissions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_permissions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
